...
def add_task():
    ...
    if task!="":
        ...
        tkinter.messagebox.showwarning(title="Warning!",message="task need to be insertred.")
def del_task():
    try:
        task_del=list_box.curselection()[0]
        list_box.delete(task_del)
    except:
         tkinter.messagebox.showwarning(title="Warning!",message="you must choose a task.")
def display_task():
    try:
